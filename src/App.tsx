...
const SpotifyPlayer: React.FC = () => {
  ...
  return (
    <motion.aside
      ...
      className={`fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 bg-[#120a1c]/95 backdrop-blur-2xl border border-rose-500/20 rounded-2xl shadow-[0_12px_45px_rgba(0,0,0,0.6)] overflow-hidden transition-all duration-300 max-w-[calc(100vw-2rem)] ${
              ...
              {playlist.map((item, idx) => (
                <div 
                  ...
                  className={`flex items-center gap-3 p-2 rounded-xl cursor-pointer transition-all ${
                    currentSong === idx ? 'bg-rose-500/20 border border-rose-500/30' : 'hover:bg-white/5'
                  }`}
                >
                  <img 
                    src={item.cover} 
        ...
        {timelineEvents.map((event, index) => {
          ...
          return (
            <motion.div
              ...
              className={`relative flex items-center justify-between mb-16 sm:mb-20 last:mb-0 ${
      ...
      <div className="text-center">
        ...
        >
          <button
            ...
            className={`px-8 sm:px-12 py-4 sm:py-6 rounded-full font-black text-lg sm:text-2xl transition-all duration-500 shadow-2xl ${
...
export default function App() {
  return (
    <div className="bg-[#0a0510] text-rose-50 font-sans selection:bg-rose-500/30 selection:text-white min-h-screen overflow-x-hidden relative">
      ...
      {/* Floating Interactive Music Player */}
      <SpotifyPlayer />
    </div>
  );
}
